...
/** relayStatusHandler – adapter mỏng: parse MQTT message rồi giao cho deviceService xử lý (ENV-FR-015) */
export async function handleRelayStatus(topicParts: string[], message: Record<string, unknown>): Promise<void> {
  try {
    ...
    logger.warn('relayStatusHandler: could not process', { topicParts, err: (err as Error).message })
  }
}
