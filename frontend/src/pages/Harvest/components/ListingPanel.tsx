import { useListingInquiries, useListingStats, useUpdateListing } from '@/hooks/useMarketplace'
...
export default function ListingPanel({ farmId, listingId }: { farmId: string; listingId: string }) {
      ...
      {inquiries && inquiries.length > 0 && (
        <div className="mt-3 flex flex-col gap-2 border-t border-charcoal/10 pt-3">
          ...
          {inquiries.slice(0, 5).map(inquiry => (
            <div key={inquiry._id} className="rounded-2xl bg-white/60 px-3 py-2 text-sm">
