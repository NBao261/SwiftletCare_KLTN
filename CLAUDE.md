...
npm run migrate:remove-sales  # one-off after SRS v1.23.0: delete SALES_STAFF users/invitations, drop SALES collections, backfill farms.members[].role (idempotent)
...
- **Farm access checks always go through `utils/farmAccess.util.ts`** (`hasFarmAccess`, `isPrimaryOwner`, `assertFarmAccess`, `assertZoneAccess`, `assertZoneInFarm`, `findFarmOrThrow`, `findZoneChainOrThrow`, `listAccessibleFarmIds`/`listAccessibleZoneIds`) — don't reimplement this in another service. **Farm Operator zone scope** (SRS v1.23.0) lives here too: `operatorZoneScope`/`hasZoneAccess` read `farms.members[].zone_ids` (empty = whole farm), `assertZoneAccess`/`listAccessibleZoneIds` already honour it, `assertRecordAccess(farmId, zoneId, user)` checks one alert/ticket/harvest, and `applyZoneScope(filter, user)` narrows any farm-keyed list query to in-scope zones + farm-level records — use these instead of filtering by role in a service.
...
| `maintenanceSchedule.job.ts` | hourly | `generateDueMaintenanceTickets()` — creates a MAINTENANCE ticket `MAINTENANCE_LEAD_DAYS` (default 3) before each schedule's `next_due_at`, then rolls it to the next cycle (TICKET-FR-013) |

**Ticket visits** (`ticket.service.ts`, `utils/visitTime.util.ts`): `scheduled_visit_at` must be in the future and within **07:00–18:00 Vietnam time** on every path that sets it — `assertValidVisitTime()` for create, `scheduleVisit` (`PUT /tickets/:id/scheduled-date`), admin-override and maintenance `next_due_at`; the maintenance job pushes a late slot to the next 07:00 with `nextVisitSlot()`. Format visit times with `formatVisitTime()` (always VN time, never the server's TZ). INSTALLATION gets its visit at creation (Farm Owner picks it); fault tickets never carry one at creation — the assignee sets it via `scheduleVisit` only once the ticket is `IN_PROGRESS` (SRS Flow 9 step 6b); MAINTENANCE only comes from schedules. Any ticket with a field visit (`requiresFieldVisit()`: INSTALLATION/MAINTENANCE or a set `scheduled_visit_at`) needs the full SAT checklist before `CLOSED`.
...
Backend roles (`Role` in `backend/src/types/domain.types.ts`): `ADMIN | FARM_OWNER | FARM_OPERATOR | TECHNICIAN`, enforced with `requireRole(...)` (`middlewares/auth.middleware.ts`). `FARM_OPERATOR` (SRS v1.23.0) is on-site staff invited by the primary Farm Owner (`POST /farms/:id/members` with `role` + `zone_ids`, scope changed via `PUT /farms/:id/members/:userId`); routes decide *whether* the role may act, `farmAccess.util` decides *which zones*. Operators may monitor, acknowledge alerts, drive relays/speaker, edit zone thresholds, create/chat tickets (cancel/rate only their own) and record harvests — never edit/delete farms, manage members, create houses/zones or publish on the marketplace. One email = one role: invitations and acceptance refuse a mismatching account.
...
**A page's folder is not an access-control statement.** `TechnicianDevicesPage` is still opened by Farm Owner, Farm Operator and Admin — `allow={OPS_ROLES}` in `routes/technician.routes.tsx` is the real permission list, and `routes/` is where you look to answer "who can reach this". The folder only says whose daily job the screen is.
...
src/
  ...
  pages/       admin/ auth/ farm-owner/ technician/ + 4 root files — ONLY `*Page.tsx`
...
AUTH, FARM, Device, Telemetry, Alert, Analytics, Ticket, System and Harvest/Marketplace all have real backend logic behind them. The SALES module and the Sales Staff role were **removed** in SRS v1.23.0 (they were only 501 stubs); the Farm Operator role was added the same release — the frontend has not caught up yet (still has `sales-staff/` code and no operator layout). **Vision/AI (bird-count)** is likewise not wired up end-to-end: the backend side exists (`mqtt/handlers/birdCount.handler.ts` → `birdCountRecord`, `GET /analytics/bird-count/daily|trends`, consumed by `useBirdCountTrends` on the Analytics page), but `ai-pipeline` isn't deployed, so nothing real publishes bird counts.
