...
- [ ] Đấu thử ESP32 + module UART-RS485 V2 (GPIO17→TXD, GPIO16→RXD, VCC 5V, GND) `[Guide 5]`
- [ ] Nạp sketch quét Modbus, đấu **từng cảm biến 1** để xác nhận Slave ID `[Config Guide Cách 1A]`
- [ ] Xác nhận ES-NOISE-01 = ID 1 `[Guide 8]`
...
- [ ] Đấu Domino → **Buck #1**, vặn biến trở + đo VOM chỉnh đúng **5V** (⚠️ trước khi cắm ESP32) `[Guide 6]`
...
- [ ] Setup PlatformIO/Arduino project cho ESP32, cấu hình UART2 (4800, 8-N-1, GPIO16/17) `[7.2]`
- [ ] Task đọc ES-NOISE-01 (reg 0x0000), quy đổi **÷10 = dB** `[ENV-FR-003, Guide 9.1]`
- [ ] Task đọc ES-CO2-01 (reg 0x0000), giá trị **trực tiếp = ppm** `[Guide 9.2]`
- [ ] Task đọc ES-NH3-01 (reg 0x0000), giá trị **trực tiếp = ppm** (bản 500ppm) `[Guide 9.3]`
- [ ] Task đọc ES-ALS-02 (reg 0x0002, 2 reg 32-bit), **×100 = Lux** (bản 200k) `[Guide 9.4]`
- [ ] Task đọc ES35-SW (reg 0+1, 2 reg), **÷10 = °C và %RH** `[Guide 9.5]`
- [ ] Gộp 5 giá trị thành 1 JSON telemetry, chu kỳ 10s (cấu hình 5-60s) `[ENV-FR-002]`
- [ ] In ra Serial Monitor để verify giá trị đúng đơn vị & hợp lý

## A4. Firmware MQTT & Onboarding (Sprint 2)

- [ ] Kết nối Wi-Fi (từ Router 4G) `[Guide 1]`
- [ ] Kết nối MQTT Broker over TLS (port 8883), QoS 1 `[7.2, SEC-NFR-001]`
- [ ] Publish telemetry topic `swiftletcare/{farmId}/{houseId}/{zoneId}/telemetry` `[ENV-FR-001, 9.2]`
- [ ] Publish heartbeat topic + relay status topic `[9.2]`
- [ ] 🤝 Nhận QR onboarding: decode {deviceId, secretKey} → gọi API M3 `[Flow 1]`
- [ ] Nhận Wi-Fi credential qua BLE/AP mode `[Flow 1]`
- [ ] Gửi heartbeat đầu tiên → backend cập nhật ONLINE `[FARM-FR-005]`
- [ ] Nhận lệnh config/update từ cloud qua MQTT `[9.2]`
...
- [ ] Logic phun sương: bật IN1 khi độ ẩm < humidity_min `[ENV-FR-010]`
- [ ] Logic quạt: bật IN3 khi nhiệt độ > temp_max HOẶC CO2 > co2_max HOẶC NH3 > nh3_max `[ENV-FR-011]`
- [ ] Logic sưởi (dự phòng): bật IN4 khi nhiệt độ < temp_min `[ENV-FR-012]`
- [ ] Publish relay state lên MQTT mỗi khi thay đổi `[ENV-FR-015]`
- [ ] Nhận lệnh Manual Override từ cloud, tạm dừng auto cho relay đó `[ENV-FR-016, 017]`
...
- [ ] Setup thư viện DFRobotDFPlayerMini, UART riêng (không đụng UART2) `[7.2]`
- [ ] Hàm điều khiển: play(track), stop, volume(0-30), loop `[ENV-FR-013b, Guide 16.3]`
- [ ] Logic lịch loa ru: đến 5-7h/17-19h → bật IN2 + DFPlayer play; hết giờ → stop + ngắt IN2 `[ENV-FR-013b]`
- [ ] 🤝 Nhận lệnh chọn bài/volume/lịch từ cloud (phối hợp M3)

## A7. Firmware Threat & Resilience (Sprint 3-4)

- [ ] Logic SENSOR_FAULT: 1 Slave ID timeout/lỗi CRC → giữ giá trị cũ + cờ stale + alert MEDIUM `[THREAT-FR-013]`
- [ ] Logic RS485_BUS_FAILURE: ≥3/5 ID timeout liên tiếp 3 chu kỳ → alert CRITICAL `[THREAT-FR-013]`
- [ ] Logic SPEAKER_FAILURE: relay loa ON + DFPlayer play nhưng dB không tăng → alert `[THREAT-FR-006]`
- [ ] Logic PUMP_DRY: relay phun sương ON nhưng ẩm không tăng sau 5p → alert `[THREAT-FR-011]`
- [ ] Logic POWER_OUTAGE: watchdog phát hiện reset bất ngờ → alert khi khởi động lại `[THREAT-FR-012]`
- [ ] Offline Resilience: tiếp tục điều khiển cục bộ khi mất Internet `[ENV-FR-014, REL-NFR-001]`
- [ ] Buffer telemetry vào SPIFFS khi mất MQTT, upload khi có mạng `[REL-NFR-003]`
- [ ] Hardware Watchdog Timer timeout 30s `[REL-NFR-002]`

## A8. Firmware OTA & Hardware Test (Sprint 5)

- [ ] OTA Update Task: nhận lệnh, tải & flash firmware mới qua MQTT `[7.2, TICKET-FR-008]`
...
- [ ] Viết README module `firmware-esp32/` `[17.2]`
...
- [ ] Khởi tạo repo Express + TypeScript `[7.4]`
- [ ] Schema `users` với enum role: ADMIN/FARM_OWNER/TECHNICIAN/SALES_STAFF `[AUTH-FR-004, 8.2]` ⚠️ 5 role, bỏ OPERATOR
- [ ] API `POST /auth/register` + OTP (email/phone) `[AUTH-FR-001]`
- [ ] API `POST /auth/login` (email/password) `[AUTH-FR-002]`
- [ ] Tích hợp OAuth2 Google `[AUTH-FR-002]`
- [ ] JWT Access (15p) + Refresh (30 ngày), API refresh/logout `[AUTH-FR-003]`
- [ ] Middleware RBAC theo 5 role `[AUTH-FR-004, 12.1]`
- [ ] API mời thành viên Farm Owner khác (Primary Owner) `[AUTH-FR-005]`
- [ ] API mời Sales Staff / tạo Technician (Admin) `[AUTH-FR-005b, 005c]`
- [ ] Guest checkout (không cần tài khoản) `[AUTH-FR-008]`
- [ ] Audit log đăng nhập/thay đổi cấu hình `[AUTH-FR-007]`
- [ ] Envelope response chuẩn + pagination + mã lỗi HTTP `[9.0]`

## C2. Farm & Device (Sprint 2)

- [ ] Schema `farms` (owner_id + members với is_primary), `houses`, `zones` `[8.2]`
- [ ] API CRUD Farm/House/Zone `[FARM-FR-001, 002]`
- [ ] API đăng ký Sensor Node qua QR `[FARM-FR-003, Flow 1]`
- [ ] Logic sinh mqttCredentials khi đăng ký device `[Flow 1]`
- [ ] Setup EMQX broker + TLS + Device Certificate `[SEC-NFR-001/005]`
- [ ] MQTT subscriber heartbeat → status ONLINE/OFFLINE `[FARM-FR-005]`
- [ ] API xem chi tiết device (firmware, uptime, RSSI) `[FARM-FR-006]`

## C3. Telemetry & Realtime (Sprint 2)

- [ ] Schema `telemetry` (temp/humidity/light/nh3/co2/sound, **bỏ h2s/tvoc**) `[8.2]` ⚠️
- [ ] Telemetry Aggregator: subscribe topic, validate, lưu MongoDB `[ENV-FR-004]`
- [ ] Đánh dấu is_anomaly cho dữ liệu ngoài ngưỡng `[ENV-FR-004]`
- [ ] Setup socket.io, event TELEMETRY_UPDATE `[ENV-FR-005, 9.3]`

## C4. Điều khiển Relay & Loa ru (Sprint 3)

- [ ] Schema `sensor_nodes` với relay_states (misting/speaker/ventilation/heating) + relay_type='GPIO' `[8.2]`
- [ ] Schema thêm speaker_schedule + audio (track/volume/playing/loop) `[8.2]`
- [ ] API cấu hình ngưỡng Zone (bỏ h2s_max/tvoc_max) `[ENV-FR-006]`
- [ ] API điều khiển relay → publish MQTT command `[ENV-FR-016]`
- [ ] API điều khiển loa ru (chọn bài/volume/lịch play/stop) `[ENV-FR-013b]`
- [ ] Logic Manual Override auto-expire 30p (scheduler) `[ENV-FR-018]`
- [ ] Lưu lịch sử override + thay đổi cấu hình `[ENV-FR-019, 009]`
- [ ] Event RELAY_UPDATE qua socket.io `[9.3]`

## C5. Alert Engine (Sprint 3-4)

- [ ] Schema `alerts` (enum type đủ SENSOR_FAULT/RS485_BUS_FAILURE/SPEAKER_FAILURE...) `[8.2]`
...
- [ ] Schema `tickets` (type/priority/status/sla/sat_checklist) `[8.2]`
...
- [ ] Schema `harvest_batches`, `nest_listings`, `contact_inquiries` `[8.2]`
...
- [ ] Rate limiting ≤100 req/phút `[SEC-NFR-004]`
- [ ] Input validation toàn bộ endpoint `[12.4]`
- [ ] bcrypt cost≥12, JWT HS256 `[SEC-NFR-003, 12.1]`
...
- [ ] **(stretch)** Module SALES: products/inventory/orders (COD-only) `[SALES-FR-001~010]`
...
- [ ] Docker Compose local (Mongo+Redis+EMQX) `[17.1]`
- [ ] GitHub Actions CI (lint + unit test mỗi PR) `[13.1]`
- [ ] File `.env.example` đầy đủ biến `[17.2]`
- [ ] Cấu trúc repo theo module + README từng repo `[17.2]`
- [ ] Import toàn bộ FR ID vào Jira/Trello
- [ ] 🤝 Endpoint `/health` cùng M3 `[OPS-NFR-003]`
...
- [ ] 🤝 Test MQTT E2E (ESP32→Broker→Backend→DB) `[13.2]`
...
- [ ] **M1+M3** — Firmware publish MQTT ↔ Backend nhận & lưu (Sprint 2) `[13.2]`
- [ ] **M1+M3+M4** — Onboarding thiết bị end-to-end: quét QR → API → ESP32 kết nối → dashboard (Sprint 2) `[Flow 1]`
- [ ] **M1+M3+M4** — Điều khiển relay/loa ru: bấm UI → API → MQTT → ESP32 phản hồi (Sprint 3) `[Flow 2]`
