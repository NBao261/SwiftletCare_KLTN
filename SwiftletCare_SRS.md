...
| POST   | `/devices/sensor-nodes/:id/audio-tracks/:trackId/play-now` **[mới v1.20.0]** | Phát thử bài này ngay lập tức, bỏ qua lịch (ENV-FR-013c(c)) — publish MQTT `audio/command`; trả lỗi nếu `synced_to_sd=false`. **Phụ thuộc firmware bổ sung `AudioManager::forcePlay()` — chưa code**                          | JWT (FARM_OWNER)       |
...
swiftletcare/{farmId}/{houseId}/{zoneId}/audio/command     # mới v1.20.0, ENV-FR-013c(c) — payload
                                                            ...
                                                            # CẦN firmware bổ sung handler +
                                                            # AudioManager::forcePlay()/forceStop()
                                                            # — CHƯA CODE (chỉ mới có ở SRS)
