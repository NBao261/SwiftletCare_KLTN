...
| TICKET-FR-009 | Nếu ticket vượt SLA xử lý mà chưa đóng, hệ thống tự động **escalate**: gửi thông báo cho Administrator và Technician dự phòng                                                                                                                                                                                                                   | Bắt buộc | ✅ Xong (`jobs/slaBreach.job.ts` quét mỗi 2 phút, bật `is_sla_breached` + ghi audit `TICKET_SLA_BREACHED`). Lưu ý `alertEscalation.job` KHÔNG phải mục này — job đó thuộc TICKET-FR-002 |
